...
* In this case, LocalEvents are used to send custom messages between code files or entities.
  + LocalEvents are messages that can be passed between scripts running on the local device. These events can pass parameterized information with them. For more information, see [Local Events](/hw-mcp-tools/documentation/hw-docs/Scripting/Events/Local%20Events.md).
