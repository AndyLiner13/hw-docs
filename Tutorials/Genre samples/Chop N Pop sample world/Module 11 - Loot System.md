...
# [Module 11 - Loot System](#module-11---loot-system)
...
![Image of loot examples](../../../_assets/images/6e09ed669f4fd7c0cfe7bb2d0a1c07e5e75e6f3f4ee3314c659caeda3fcda143.png)
...
### [Loot at world startup](#loot-at-world-startup)
...
### [Loot at runtime](#loot-at-runtime)
...
2. **Spawned assets**: If you are spawning in assets, create a LootTableInstance reference objec&#x74;**.** The LootTableInstance reference object hosts `LootTableAssets.ts`, which defines the loot types in the system. Specify the loot types in the script.
3. **Non-spawned assets**: Create group of loot entitie&#x73;**.** For assets that are not spawned, in the Hierarchy panel, create a reference object for the Group of assets.
4. Create and deploy a loot item type, which must include:
   1. A root reference object.
      ...
      2. Specify script parameters. For the parentPool parameter, select the root reference object you just created.
   2. Sub-objects:
      1. A model
      2. A highlight particle FX
      3. A pickup sound
5. This loot type must be stored outside of the game area.
6. Duplicate this item in the same location for as many instances of it as are needed in your world.
7. **Create system nodes**: In the equivalent of WorldInstances node in you world, create nodes for the following (attached script):
...
### [Add or remove loot entities](#add-or-remove-loot-entities)
...
### [Add new loot type](#add-new-loot-type)
...
1. Create a loot asset composed of:
   ...
   4. Sound (optional)
2. Add references to it in the LootTableInstance script properties, modifying odds for it to occur.
