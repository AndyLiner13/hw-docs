---
source: https://developers.meta.com/horizon-worlds/learn/documentation/tutorial-worlds/genre-samples/sim-tycoon-tutorial/module-4-pickaxe
---

# [Module 4 - Pickaxe (Tool)](#module-4---pickaxe-tool)
...
### [Pickaxe properties](#pickaxe-properties)

**name** – The string identifier of the tool. The name is used to track the player’s equipped tools and resource interactions. **toolType** – Signature of the tool type. Resource nodes validate this type before extraction is allowed. **primaryResource** – The resource type name that the pickaxe is most effective against. **primaryMult** – Multiplier applied when mining the correct (matching) resource type. **secondaryMult** – Multiplier applied when mining a non-matching resource type.
...
1. Add `Pickaxe.ts` to your project.
2. Create a pickaxe grabbable entity using your 3D model.
3. Attach `Pickaxe.ts` to the entity.
4. Set properties if you want custom resource types or multipliers.
   1. In the properties, set ‘Collision Events From’ to ‘Objects Tagged’
   2. Set object tag to ‘RESOURCE’
5. Make sure resource nodes use `ResourceNode.ts` or `GlobalResourceNode.ts`
6. If you are adding a new tool, make sure to add it to the save game data (see below for more information).
