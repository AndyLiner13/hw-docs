---
source: https://developers.meta.com/horizon-worlds/learn/documentation/tutorial-worlds/feature-samples/ai-conversation-tutorial/module-6-exercise-add-game-events
---

# [Module 6 - Exercise - Add Game Events](#module-6---exercise---add-game-events)
...
## [Adding a player score event](#adding-a-player-score-event)

Below is an example of scripting using the Conversation API to add a new game event that responds to player scoring:

### [Step 1: Add the event handler function](#step-1-add-the-event-handler-function)

1. Add this function to the `NpcManager` class:
...
### [Step 2: Hook the event to game logic](#step-2-hook-the-event-to-game-logic)

1. Add this line of code to `SimPlayer.setPlayerScore` function in `SimPlayer.ts`:

```typescript
    NpcManager.onPlayerSetScore( this.player, newScore );
```

### [Step 3: Test and experiment](#step-3-test-and-experiment)

1. Save the code and play the world. Experiment with which calls and which parameters are used in `onPlayerSetScore` to see the various ways the Conversation API will react to these inputs.
...
## [Experiment with different approaches](#experiment-with-different-approaches)
