...
- Player activates their tool near a resource node
- Tool deals damage to the node based on tool tier and efficiency
- Node health decreases by the damage amount
- If node health reaches zero, it generates resources for the player
...
- Create new resource node entities in the world
- Configure the ResourceNode component with new resource type
- Update the inventory system to track the new resource
- Add the new resource to store conversion rates
- Update HUD displays to show the new resource
...
- Group multiple nodes of the same type in specific areas
- Vary health and respawn timers within clusters
- Add environmental theming to match resource types
- Consider adding rare “bonus” nodes with higher rewards
