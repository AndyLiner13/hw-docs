...
# [Multiple Materials per Mesh](#multiple-materials-per-mesh)
...
### [Forcing a material slot order](#forcing-a-material-slot-order)
...
## [Referencing material slots in the scripting API](#referencing-material-slots-in-the-scripting-api)
...
1. **Masked material:** Either all materials must be masked material or none can be. If a subset of submeshes are masked, the shadow and reflection intensity will be incorrect, treating them as if none are masked.
2. **VertexTextureColor:** Either all materials must be VertexTextureColor or none can be. Using a mix of VertexTextureColor and other shading models will lead to unexpected behaviors.
3. **UI Optimized only:** Swapping methods are supported only for UI Optimized (`_UIO`) materials.
