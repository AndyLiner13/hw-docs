...
One straightforward way to achieve this is to duplicate the entity and set the visibility of each entity so that it is only visible to one player. We actually recommend using the [Local Mode](/hw-mcp-tools/documentation/hw-docs/Desktop%20editor/Custom%20UI/Local%20Mode%20Custom%20UI%20Scripts.md) when adopting this approach. Please see a more detailed discussion and examples in the [Local Mode](/hw-mcp-tools/documentation/hw-docs/Desktop%20editor/Custom%20UI/Local%20Mode%20Custom%20UI%20Scripts.md) section. But there are cases where we don’t want to have multiple UI gizmos, and want to keep one single panel that is publicly visible. Custom UI feature allows us to display different content to each player *on the same UI Gizmo* . This is achieved by only updating the value of a Binding to certain players, thus creating a player-specific value for them.
...
Therefore, the correct implementation for the button with hover state needs to take [the acting player](/hw-mcp-tools/documentation/hw-docs/Desktop%20editor/Custom%20UI/Build%20an%20interactive%20custom%20UI.md) into account, and only manipulate the player specific value:
