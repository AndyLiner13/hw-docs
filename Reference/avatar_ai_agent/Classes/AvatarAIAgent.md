...
| **spawnAgentPlayer()** | Spawns a player controlled by the `AvatarAIAgent` object.      Signature  ``` spawnAgentPlayer(): Promise<AgentSpawnResult>; ```  Returns  Promise<[AgentSpawnResult](../Enumerations/AgentSpawnResult.md)>  A promise describing the results of the spawn operation. |
