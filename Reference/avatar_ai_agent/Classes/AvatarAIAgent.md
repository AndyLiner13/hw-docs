...
| **spawnAgentPlayer()** | Spawns a player controlled by the `AvatarAIAgent` object.      Signature  ``` spawnAgentPlayer(): Promise<AgentSpawnResult>; ```  Returns  Promise<[AgentSpawnResult](https://developers.meta.com/horizon-worlds/reference/2.0.0/avatar_ai_agent_agentspawnresult)>  A promise describing the results of the spawn operation. |
