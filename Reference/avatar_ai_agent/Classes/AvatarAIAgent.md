...
| **spawnAgentPlayer()** | Spawns a player controlled by the `AvatarAIAgent` object.      Signature  ``` spawnAgentPlayer(): Promise<AgentSpawnResult>; ```  Returns  Promise<[AgentSpawnResult](/hw-mcp-tools/documentation/hw-docs/Reference/avatar_ai_agent/Enumerations/AgentSpawnResult.md)>  A promise describing the results of the spawn operation. |
