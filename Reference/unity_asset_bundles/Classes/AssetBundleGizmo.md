...
| **getReference(name, throwIfNotExist)** | Gets an AssetBundleInstanceReference for an exposed GameObject in the loaded prefab. These need to be specified at export time in the HorizonUnityAssetReferences MonoBehaviour.      Signature  ``` getReference(name: string, throwIfNotExist: boolean): AssetBundleInstanceReference; ```  Parameters  name: string  The name of the GameObject.  throwIfNotExist: boolean  Indicates whether to throw an exception if the GameObject does not exist.  Returns  [AssetBundleInstanceReference](https://developers.meta.com/horizon-worlds/reference/2.0.0/unity_asset_bundles_assetbundleinstancereference)  A reference to the GameObject.  Exceptions  Thrown if throwIfNotExist is true and the GameObject does not exist. |
| **getRoot()** | Gets an AssetBundleInstanceReference for the main GameObject.      Signature  ``` getRoot(): AssetBundleInstanceReference; ```  Returns  [AssetBundleInstanceReference](https://developers.meta.com/horizon-worlds/reference/2.0.0/unity_asset_bundles_assetbundleinstancereference)  A reference to the main GameObject. |
