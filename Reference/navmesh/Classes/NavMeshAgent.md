...
| **maxSpeed** | The max travel speed for the agent.      Signature  ``` maxSpeed: HorizonProperty<number>; ```  Remarks  To change how fast the agent reaches its max speed, use the [acceleration](NavMeshAgent.md#acceleration) property. Default: 5 meters per second |
...
| **turnSpeed** | The rate in degrees pers second, at which the agent rotates towards its desired orientation.      Signature  ``` turnSpeed: HorizonProperty<number>; ```  Remarks  The agent's desired orientation is determined by its [alignmentMode](NavMeshAgent.md#alignmentmode) property. Default: 120 degrees per second |
