...
| **registerCallback(callback)** | Registers a callback that is called when the input is pressed or released. For analog inputs, a pressed state corresponds to an axis value greater than 0.5 or lesser than -0.5.      Signature  ``` registerCallback(callback: PlayerInputStateChangeCallback): void; ```  Parameters  callback: [PlayerInputStateChangeCallback](https://developers.meta.com/horizon-worlds/reference/2.0.0/core_playerinputstatechangecallback)  The callback that is called when the pressed state changes.  Returns  void |
