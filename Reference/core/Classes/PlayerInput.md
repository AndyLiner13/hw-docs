...
For more information about binding player input, see the [Custom Input API](/hw-docs/create-for-web-and-mobile/typescript-apis-for-mobile/custom-input-api) guide.
...
| **registerCallback(callback)** | Registers a callback that is called when the input is pressed or released. For analog inputs, a pressed state corresponds to an axis value greater than 0.5 or lesser than -0.5.      Signature  ``` registerCallback(callback: PlayerInputStateChangeCallback): void; ```  Parameters  callback: [PlayerInputStateChangeCallback](/hw-docs/Reference/core/Type%20Aliases/PlayerInputStateChangeCallback.md)  The callback that is called when the pressed state changes.  Returns  void |
