...
### [setMaterial(materialAsset, options)](#setmaterialmaterialasset-options)
...
A material asset from the asset library.
...
### [setMesh(mesh, options)](#setmeshmesh-options)
...
The new custom model asset to use in the world. You must use a custom model asset that was consumed as a custom model in the asset pipeline. You cannot use a custom model asset that is saved as an asset within Meta Horizon Worlds.

options: [SetMeshOptions](../Type%20Aliases/SetMeshOptions.md)

true if players can decide to use the new material that comes with the new custom model; false to use the current material.
...
### [setTexture(texture, options)](#settexturetexture-options)
...
texture: [TextureAsset](TextureAsset.md)

The asset containing the texture to apply. The asset must be a texture asset that has been consumed as a texture in the asset pipeline.

options: [SetTextureOptions](../Type%20Aliases/SetTextureOptions.md)

*(Optional)* Indicates the players to apply the texture for.
...
This API should only be applied to a custom model entity that uses a texture based material. Additionally, non-interactive (Motion property set to None) entities may not update textures if the material shader is GI lit. Otherwise, this call does not take effect and throws an error at runtime.
...
A string representation of the `MeshEntity`.
