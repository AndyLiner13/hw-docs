...
| **visible** | Indicates whether players with permission can see the entity. true if players with permission can see the entity; false if no players can see the entity.      Signature  ``` visible: HorizonProperty<boolean>; ```  Examples  ``` const wasVisible: boolean = cubeEntity.visible.get(); cubeEntity.visible.set(!wasVisible); ```  Remarks  You can set which players have permission using [Entity.setVisibilityForPlayers()](/hw-mcp-tools/documentation/hw-docs/Reference/core/Classes/Entity.md#setvisibilityforplayers). It's important to note that if any parent entity has its visibility set to false, the child entity will also be invisible regardless of its own visibility setting. |
...
| **lookAt(target, up)** | Rotates an entity to look at a point.      Signature  ``` lookAt(target: Vec3, up?: Vec3): void; ```  Parameters  target: [Vec3](/hw-mcp-tools/documentation/hw-docs/Reference/core/Classes/Vec3.md)  The target for the entity to look at.  up: [Vec3](/hw-mcp-tools/documentation/hw-docs/Reference/core/Classes/Vec3.md)  *(Optional)* The up direction of the rotation. The default value is [Vec3.up](/hw-mcp-tools/documentation/hw-docs/Reference/core/Classes/Vec3.md#up).  Returns  void |
