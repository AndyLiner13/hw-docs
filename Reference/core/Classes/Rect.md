...
|  |  |
| --- | --- |
| **clone()** | Clones a Rectangle's values into a mutable Rect.      Signature  ``` clone(): Rect; ```  Returns  [Rect](Rect.md)  A mutable Rect with the same x,y,width,height values. |
| **copy(rect)** | Copies the specified Rect (x, y, width, height) into this.      Signature  ``` copy(rect: Rect): this; ```  Parameters  rect: [Rect](Rect.md)  The Rectangle to copy from.  Returns  this  A reference to this after the values have been copied. |
