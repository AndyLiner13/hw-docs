...
If you have actions to perform after playback of an audio source completes, you can listen for the `OnAudioCompleted` [CodeBlockEvent](/hw-docs/Reference/core/Variables/CodeBlockEvents.md).
...
|  |  |
| --- | --- |
| **pause(audioOptions)** | Pauses an AudioGizmo sound.      Signature  ``` pause(audioOptions?: AudioOptions): void; ```  Parameters  audioOptions: [AudioOptions](/hw-docs/Reference/core/Type%20Aliases/AudioOptions.md)  *(Optional)* Controls how the audio is paused.  Returns  void  Examples  ``` const soundGizmo = this.props.sfx.as(hz.AudioGizmo); const audioOptions: AudioOptions = {fade: 1, players: [player1, player2]}; soundGizmo.pause(audioOptions); ``` |
| **play(audioOptions)** | Plays an AudioGizmo sound.      Signature  ``` play(audioOptions?: AudioOptions): void; ```  Parameters  audioOptions: [AudioOptions](/hw-docs/Reference/core/Type%20Aliases/AudioOptions.md)  *(Optional)* Controls how the audio is played.  Returns  void  Examples  ``` const soundGizmo = this.props.sfx.as(hz.AudioGizmo); const audioOptions: AudioOptions = {fade: 1, players: [player1, player2]}; soundGizmo.play(audioOptions); ``` |
| **stop(audioOptions)** | Stops an AudioGizmo sound.      Signature  ``` stop(audioOptions?: AudioOptions): void; ```  Parameters  audioOptions: [AudioOptions](/hw-docs/Reference/core/Type%20Aliases/AudioOptions.md)  *(Optional)* Controls how the audio is played.  Returns  void  Examples  ``` const soundGizmo = this.props.sfx.as(hz.AudioGizmo); const audioOptions: AudioOptions = {fade: 1, players: [player1, player2]}; soundGizmo.stop(audioOptions); ``` |
