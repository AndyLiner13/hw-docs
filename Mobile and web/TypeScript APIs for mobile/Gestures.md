...
You use the [Gestures](/hw-docs/Reference/mobile_gestures_gestures) class to listen for these gesture events on a component owned by the local player.

## Step 1: Setup and initialization

To use the gestures API, you need to:

* Import the [Gestures](/hw-docs/Reference/mobile_gestures_gestures) class from `horizon/mobile_gestures`.
...
Each event is a [GestureEvent](/hw-docs/Reference/mobile_gestures_gestureevent) that you connect to with a callback function receiving gesture data.
...
| --- | --- | --- |
| Tap | [TapEventData](/hw-docs/Reference/mobile_gestures_tapeventdata) | `touches` array containing [TouchState](/hw-docs/Reference/mobile_gestures_touchstate) objects |
| Long Tap | [LongTapEventData](/hw-docs/Reference/mobile_gestures_longtapeventdata) | `touches` array containing [TouchState](/hw-docs/Reference/mobile_gestures_touchstate) objects |
| Swipe | [SwipeEventData](/hw-docs/Reference/mobile_gestures_swipeeventdata) | `touches` array, `swipeDirection` enum indicating direction of the swipe (`up`, `down`, `left`, `right`, or `none`) |
| Pan | [PanEventData](/hw-docs/Reference/mobile_gestures_paneventdata) | `touches` array, `pan` vector (`Vec3`) reperesenting the movement delta in screen space |
| Pinch | [PinchEventData](/hw-docs/Reference/mobile_gestures_pincheventdata) | `touches` array, `scale` (number), `rotate` (radians) |

#### Touch state details

Each touch in a `touches` array has the following [state information](/hw-docs/Reference/mobile_gestures_touchstate):

| Propert Name | Data Type | Description |
| --- | --- | --- |
| `phase` | [TouchPhase](/hw-docs/Reference/mobile_gestures_touchphase) | The current phase of the touch: `'start'`, `'move'`, or `'end'`. Indicates if the touch just began, is moving, or ended. |
| `start` | [TouchInfo](/hw-docs/Reference/mobile_gestures_touchinfo) | The state of the touch when it first started. Includes position and timestamp. |
| `previous` | [TouchInfo](/hw-docs/Reference/mobile_gestures_touchinfo) | The state of the touch during the previous frame. Useful for calculating movement deltas. |
| `current` | [TouchInfo](/hw-docs/Reference/mobile_gestures_touchinfo) | The state of the touch during the current frame. Contains the latest position and timestamp. |
...
You can customize gesture detection thresholds by passing [options](/hw-docs/Reference/mobile_gestures_gesturesoptions) to the `Gestures` constructor:
